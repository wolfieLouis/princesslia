...
async function sbGet(table, orderCol) {
  ...
  if (error) { setSync('❌ Error: ' + error.message, 'err'); return []; }
  return data || [];
}
...
document.getElementById('nav').addEventListener('click', e => {
  ...
  const map = { dev:renderDev, agenda:renderAgenda, verses:renderVerses,
...
async function renderDev() {
  ...
  const refCards = reflections.map(r =>
    ...
    + '<div class="refl-text">' + x(r.text) + '</div></div>'
  ).join('');
  ...
  document.getElementById('refl-btn').onclick = async () => {
    ...
    if (await sbInsert('reflections', { verse: v.ref + ' — ' + v.text, text: t, date: new Date().toLocaleString('es-ES') })) renderDev();
  };
...
function renderPrayers(items, pcats) {
  ...
  items.forEach(p => {
    ...
    d.innerHTML =
      '<button class="tog-p tog-circle" data-id="' + p.id + '" style="border:2px solid '
      + (p.done?'#ffd700':'#e91e8c') + ';color:' + (p.done?'#ffd700':'transparent') + '">'
      + (p.done?'✓':'') + '</button>'
      + '<div style="flex:1"><div style="font-size:13px;text-decoration:' + (p.done?'line-through':'none')
      + ';color:' + (p.done?'#a06090':'#fce4ff') + '">' + x(p.text) + '</div>'
  ...
  list.querySelectorAll('.edit-p').forEach(b => b.onclick = async () => {
    const pc = await sbGetCfg('pcats', ['Familia','Estudios','Salud','Iglesia','Personal']);
    openPrayerModal(items.find(p => p.id == b.dataset.id), pc);
  });
...
async function openMatModal(materias) {
  ...
  document.getElementById('m-matsave').onclick = async () => {
    ...
    await sbSetCfg('materias', materias); closeModal(); renderAgenda();
  };
...
async function renderVerses() {
  ...
  document.getElementById('btn-vcats').onclick = () => openCatModal('vcats', 'Categorías de versículos', vcats, renderVerses);
...
async function openCatModal(cfgKey, title, current, onDone) {
  ...
  const render = () => {
    ...
    document.getElementById('m-catsave').onclick = async () => {
      ...
      await sbSetCfg(cfgKey, nc); closeModal(); if (onDone) onDone();
    };
...
function openRutinaModal(r) {
  ...
  const exRow = (e) =>
    ...
    + '</div></div>';
  const exRows = (r ? r.ejercicios : []).map(e => exRow(e)).join('');
  ...
  document.getElementById('add-ex').onclick = () => {
    document.getElementById('ex-rows').insertAdjacentHTML('beforeend', exRow({name:'',series:3,reps:10,peso:''}));
  };
  document.getElementById('m-rtsave').onclick = async () => {
    ...
    document.querySelectorAll('.ex-row').forEach(row => {
      ...
      ejercicios.push({ name:n,
        series: parseInt(row.querySelector('.ex-ser').value)||3,
        reps:   parseInt(row.querySelector('.ex-rep').value)||10,
        peso:   row.querySelector('.ex-peso').value.trim() });
    });
    let ok;
...
function openLogroModal() {
  ...
  document.getElementById('m-lsave').onclick = async () => {
    ...
    const pr = parseInt(document.getElementById('m-lprog').value)||0;
    const to = parseInt(document.getElementById('m-ltot').value)||1;
    const u  = document.getElementById('m-lunit').value.trim()||'veces';
    if (await sbInsert('logros_gym', { title:t, cat:c, prog:pr, total:to, unit:u })) { closeModal(); renderGym(); }
  };
  document.getElementById('m-lcancel').onclick = closeModal;
}
